...
export default function Menucontrol() {
  const SWEETS_DATA_FROM_LOCAL_STORAGE1 = JSON.parse(
    localStorage.getItem("SWEETS_DATA_loc")
  );
  const FOODS_DATA_FROM_LOCAL_STORAGE1 = JSON.parse(
    localStorage.getItem("FOODS_DATA_loc")
  );
  const DRINKS_DATA_FROM_LOCAL_STORAGE1 = JSON.parse(
  ...
  if (!sweet && !foods && !drink) {
    ...
    return (
      <div>
        ...
        <MenuControlComponent
          data={SWEETS_DATA_FROM_LOCAL_STORAGE1}
    ...
    return (
      <div>
        {elements}
        <MenuControlComponent
          data={FOODS_DATA_FROM_LOCAL_STORAGE1}
    ...
    return (
      <div>
        {elements}
        <MenuControlComponent
          data={DRINKS_DATA_FROM_LOCAL_STORAGE1}
